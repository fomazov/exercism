...
export function simpleTicketStatus(tickets, ticketId) {
  const INVALID_COPY = "invalid ticket !!!";
  const value = tickets[ticketId];
  if (value === undefined) return INVALID_COPY;
  return value === null ? INVALID_COPY : value;
}
...
export function gtcVersion(visitor) {
  for (const [key, value] of Object.entries(visitor)) {
    if (key === "gtc") {
      return value === null ? "not sold" : visitor[key].version;
    }
  }
}
